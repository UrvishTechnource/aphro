    ...
    protected function getIndexedFields($changes) {
        ...
        if (empty($changes['load_defaults'])) {
            ...
            return [
                ...
                'use_filter_title_in_url' => new Zend_Db_Expr($db->quoteInto("COALESCE(`fge`.`use_filter_title_in_url`, ?)", $this->configuration->getDefaultUseFilterTitleInUrl())),
                ...
                'include_in_canonical_url' => new Zend_Db_Expr($db->quoteInto("COALESCE(`fge`.`include_in_canonical_url`, ?)", $this->configuration->getDefaultIncludeInCanonicalUrl())),
                'force_no_index' => new Zend_Db_Expr($db->quoteInto("COALESCE(`fge`.`force_no_index`, ?)", $this->configuration->getDefaultForceNoIndex())),
                'force_no_follow' => new Zend_Db_Expr($db->quoteInto("COALESCE(`fge`.`force_no_follow`, ?)", $this->configuration->getDefaultForceNoFollow())),
                'include_in_meta_title' => new Zend_Db_Expr($db->quoteInto("COALESCE(`fge`.`include_in_meta_title`, ?)", $this->configuration->getDefaultIncludeInMetaTitle())),
                'include_in_meta_description' => new Zend_Db_Expr($db->quoteInto("COALESCE(`fge`.`include_in_meta_description`, ?)", $this->configuration->getDefaultIncludeInMetaDescription())),
                'include_in_meta_keywords' => new Zend_Db_Expr($db->quoteInto("COALESCE(`fge`.`include_in_meta_keywords`, ?)", $this->configuration->getDefaultIncludeInMetaKeywords())),
                'include_in_sitemap' => new Zend_Db_Expr($db->quoteInto("COALESCE(`fge`.`include_in_sitemap`, ?)", $this->configuration->getDefaultIncludeInSitemap())),
            ];
        }
        else {
            ...
            return [
                ...
                'use_filter_title_in_url' => new Zend_Db_Expr($db->quoteInto("?", $this->configuration->getDefaultUseFilterTitleInUrl())),
                'url_part' => new Zend_Db_Expr($db->quoteInto("?", $this->configuration->getDefaultUrlPart())),
                'position_in_url' => new Zend_Db_Expr("$position"),
                'include_in_canonical_url' => new Zend_Db_Expr($db->quoteInto("?", $this->configuration->getDefaultIncludeInCanonicalUrl())),
                'force_no_index' => new Zend_Db_Expr($db->quoteInto("?", $this->configuration->getDefaultForceNoIndex())),
                'force_no_follow' => new Zend_Db_Expr($db->quoteInto("?", $this->configuration->getDefaultForceNoFollow())),
                'include_in_meta_title' => new Zend_Db_Expr($db->quoteInto("?", $this->configuration->getDefaultIncludeInMetaTitle())),
                'include_in_meta_description' => new Zend_Db_Expr($db->quoteInto("?", $this->configuration->getDefaultIncludeInMetaDescription())),
                'include_in_meta_keywords' => new Zend_Db_Expr($db->quoteInto("?", $this->configuration->getDefaultIncludeInMetaKeywords())),
                'include_in_sitemap' => new Zend_Db_Expr($db->quoteInto("?", $this->configuration->getDefaultIncludeInSitemap())),
            ];
        }
    }
