...
class Login extends \Plumrocket\SocialLoginFree\Controller\AbstractAccount
{
  ...
  public function execute()
  {
    ...
    //Check for cancel account Technource-Rahul-17-04-2017

    if($customerId = $model->getCustomerIdByUserId()){
      $customerId = $model->getCustomerIdByUserId();
      $customerRepository = $this->_objectManager->get('Magento\Customer\Api\CustomerRepositoryInterface');
      $customer = $customerRepository->getById($customerId);


      $cust = $customer->getCustomAttribute('canc_acc')->getValue();

      if($cust==0)
      {

       $message = __('Sorry! Your account is Cancelled Please Contact our admin');
       $this->messageManager->addError($message);

      return $this->_wclose_reload();
